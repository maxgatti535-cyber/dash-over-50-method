...
const AICoach: React.FC<AICoachProps> = ({ initialPrompt, clearInitialPrompt }) => {
  ...
  const addBPContext = () => {
    ...
    if (readings.length === 0) {
        setInput("CONTEXT: I have no BP readings logged yet.\n\n" + input);
  ...
  const sendMessage = async (messageText: string): Promise<boolean> => {
    ...
    try {
        ...
        // Determine API Key (User preference overrides system default)
        const storedKey = getLocalStorageItem('system.userApiKey', '');
        const userKey = typeof storedKey === 'string' ? storedKey : '';
        // FIX: Usa process.env.API_KEY invece di __API_KEY__
        const effectiveKey = userKey.trim() !== '' ? userKey : process.env.API_KEY;
        ...
        if (error instanceof Error) {
            ...
            if (lowerCaseError.includes('permission') || lowerCaseError.includes('denied')) {
                ...
                errorMessageText = "The AI service usage limit has been reached. Please try again later.";
                 ...
                 errorMessageText = "System Error: API Key configuration is missing. Please check settings or environment.";
            } else if (lowerCaseError.includes('no api key')) {
                 errorMessageText = "API Key missing. Please add your key in Settings -> System.";
            } else if (lowerCaseError.includes('not defined')) {
                 errorMessageText = "System Error: API Key configuration issue. Please check environment variables.";
            }
        }
