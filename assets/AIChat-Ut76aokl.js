import{r as w,g as p,j as h,m as ve}from"./index-BepDpgsK.js";var K;(function(e){e.STRING="string",e.NUMBER="number",e.INTEGER="integer",e.BOOLEAN="boolean",e.ARRAY="array",e.OBJECT="object"})(K||(K={}));/**
 * @license
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const ie=["text","inlineData","functionCall","functionResponse","executableCode","codeExecutionResult"],Be={user:["text","inlineData"],function:["functionResponse"],model:["text","functionCall","executableCode","codeExecutionResult"],system:["text"]};function Ye(e){let t=!1;for(const n of e){const{role:s,parts:o}=n;if(!t&&s!=="user")throw new m(`First content should be with role 'user', got ${s}`);if(!J.includes(s))throw new m(`Each item should include role field. Got ${s} but valid roles are: ${JSON.stringify(J)}`);if(!Array.isArray(o))throw new m("Content should have 'parts' property with an array of Parts");if(o.length===0)throw new m("Each Content should have at least one part");const i={text:0,inlineData:0,functionCall:0,functionResponse:0,fileData:0,executableCode:0,codeExecutionResult:0};for(const r of o)for(const u of ie)u in r&&(i[u]+=1);const a=Be[s];for(const r of ie)if(!a.includes(r)&&i[r]>0)throw new m(`Content with role '${s}' can't contain '${r}' part`);t=!0}}function ae(e){var t;if(e.candidates===void 0||e.candidates.length===0)return!1;const n=(t=e.candidates[0])===null||t===void 0?void 0:t.content;if(n===void 0||n.parts===void 0||n.parts.length===0)return!1;for(const s of n.parts)if(s===void 0||Object.keys(s).length===0||s.text!==void 0&&s.text==="")return!1;return!0}/**
 * @license
 * Copyright 2024 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class We{constructor(t){this.apiKey=t}getGenerativeModel(t,n){if(!t.model)throw new m("Must provide a model name. Example: genai.getGenerativeModel({ model: 'my-model-name' })");return new ce(this.apiKey,t,n)}getGenerativeModelFromCachedContent(t,n,s){if(!t.name)throw new T("Cached content must contain a `name` field.");if(!t.model)throw new T("Cached content must contain a `model` field.");const o=["model","systemInstruction"];for(const a of o)if(n!=null&&n[a]&&t[a]&&(n==null?void 0:n[a])!==t[a]){if(a==="model"){const r=n.model.startsWith("models/")?n.model.replace("models/",""):n.model,u=t.model.startsWith("models/")?t.model.replace("models/",""):t.model;if(r===u)continue}throw new T(`Different value for "${a}" specified in modelParams (${n[a]}) and cachedContent (${t[a]})`)}const i=Object.assign(Object.assign({},n),{model:t.model,tools:t.tools,toolConfig:t.toolConfig,systemInstruction:t.systemInstruction,cachedContent:t});return new ce(this.apiKey,i,s)}}const le={checkin:"Daily Check-In",plan3d:"3-Day DASH Plan (~1,800 mg sodium/day)",labels:"Read Food Labels (sodium)",movement:"Gentle Movement (20 min/day)",eatout:"Eat Out Low Sodium"},ze=({initialPrompt:e,clearInitialPrompt:t})=>{const[n,s]=w.useState([{text:"Hello! I'm your DASH Coach. How can I help you today?",sender:"ai"}]),[o,i]=w.useState(""),[a,r]=w.useState(!1),[u,v]=w.useState([]),O=w.useRef(null),y=w.useRef(null);w.useEffect(()=>{const l=()=>{const S=p("preferences.coachQuickActions",{checkin:!0,plan3d:!0,labels:!0,movement:!0,eatout:!0}),N=Object.keys(le).filter(_=>S[_]).map(_=>le[_]);v(N)};return l(),window.addEventListener("settings-changed",l),()=>{window.removeEventListener("settings-changed",l)}},[]),w.useEffect(()=>{var l;(l=O.current)==null||l.scrollIntoView({behavior:"smooth"})},[n]),w.useEffect(()=>{e&&(d(e),t&&t())},[e]);const f=()=>{var _,L;let l=p("dash_bp_readings",[]);if(l.length===0){i(c=>`CONTEXT: I have no BP readings logged yet.

`+c.trim()),(_=y.current)==null||_.focus();return}l.sort((c,C)=>new Date(C.date+"T"+C.time).getTime()-new Date(c.date+"T"+c.time).getTime());const S=l.slice(0,5);let N=`Here are my most recent blood pressure readings for context:
`;S.forEach(c=>{const C=new Date(c.date+"T"+c.time).toLocaleDateString("en-US",{month:"short",day:"numeric"});N+=`- ${C}: ${c.systolic}/${c.diastolic} mmHg
`}),i(c=>N+`
`+c.trim()),(L=y.current)==null||L.focus()},d=async l=>{var _,L;if(!l.trim())return!0;const S={text:l,sender:"user"};s(c=>[...c,S]),r(!0);let N=!1;try{const c={name:p("profile.name",""),age:p("profile.age",""),sex:p("profile.sex",""),heightFt:p("profile.heightFt",""),heightIn:p("profile.heightIn",""),heightCm:p("profile.heightCm",""),weight:p("profile.weight",""),medicalConditions:p("profile.medicalConditions",""),units:p("preferences.units","us"),sodiumTarget:p("preferences.sodiumTargetMg",1800),exerciseLevel:p("preferences.exerciseLevelDefault","beginner")},C=p("dash_medications_v2",[]);let g=`
...
`,C.length>0?(g+=`
Medications:
`,C.forEach(R=>{g+=`- ${R.name} ${R.dose}${R.unit}
`})):g+=`
...
Include a brief disclaimer if answering medical or drug-related topics.`,D="AIzaSyASG6xI0ys4wwRUZhvQjpWUm5BoOvo8aNo";console.log("DEBUG: API Key present?",!!D,D?`Length: ${D.length}`:"Missing");const me=new We(D),pe=`${Ee}

${g}

${l}`,F=["gemini-2.5-flash","gemini-2.0-flash","gemini-1.5-flash"];console.log("AIChat attempting models:",F);let U;for(const R of F)try{const Y=(L=(_=(await me.getGenerativeModel({model:R}).generateContent(pe)).response)==null?void 0:_.text)==null?void 0:L.call(_);if(Y){console.log(`Model ${R} succeeded.`),U=Y;break}}catch(B){console.warn(`Model ${R} failed:`,B)}if(!U)throw new Error("All models failed to generate a response.");const Ce={text:U,sender:"ai"};s(R=>[...R,Ce]),N=!0}catch(c){console.error("Gemini API error:",c);let C="Sorry, I'm having trouble connecting right now. Please try again later.";if(c instanceof Error){const I=c.message.toLowerCase();I.includes("permission")||I.includes("denied")?C="It looks like there's a permission issue with the AI service. Please contact support.":I.includes("quota")?C="The AI service usage limit has been reached. Please try again later.":I.includes("model")&&(I.includes("not found")||I.includes("unavailable"))?C="The AI model is currently unavailable. Please try again later.":I.includes("api key")&&(C="System Error: API Key configuration is missing. Please check your environment settings.")}const g={text:C,sender:"ai"};s(I=>[...I,g])}finally{r(!1)}return N},E=async()=>{const l=o;if(!l.trim())return;i(""),await d(l)||i(l)},A=l=>{d(l)};return h.jsxs("div",{className:"flex flex-col bg-surface h-full",children:[h.jsxs("div",{className:"flex-grow p-2 space-y-4 overflow-y-auto",children:[n.map((l,S)=>h.jsx("div",{className:`flex ${l.sender==="user"?"justify-end":"justify-start"}`,children:h.jsx("div",{className:`p-3 rounded-2xl max-w-xs md:max-w-md shadow-sm ${l.sender==="user"?"bg-brandPrimary text-white rounded-br-none":"bg-brandPrimaryTint text-textPrimary rounded-bl-none"}`,children:h.jsx("div",{className:"prose text-lg",dangerouslySetInnerHTML:{__html:ve(l.text)}})})},S)),a&&h.jsx("div",{className:"flex justify-start",children:h.jsx("div",{className:"p-3 rounded-2xl bg-brandPrimaryTint text-textPrimary rounded-bl-none",children:h.jsx("span",{className:"animate-pulse text-lg",children:"● ● ●"})})}),h.jsx("div",{ref:O})]}),h.jsxs("div",{className:"p-2 bg-surface border-t border-border",children:[h.jsx("div",{className:"px-2 pb-2",children:h.jsx("button",{onClick:f,className:"w-full text-left text-accentBlue font-semibold text-lg py-2 px-3 rounded-lg border-2 border-dashed border-accentBlue hover:bg-accentBlue/10 transition-colors",children:"+ Include Recent BP Readings in Message"})}),h.jsx("div",{className:"flex overflow-x-auto whitespace-nowrap gap-2 mb-3 pb-2",children:u.map(l=>h.jsx("button",{onClick:()=>A(l),className:"flex-shrink-0 px-4 py-3 bg-brandPrimaryTint text-brandPrimary rounded-full text-base font-medium hover:bg-brandAccent/50 transition-colors min-h-[48px]",children:l},l))}),h.jsxs("div",{className:"flex items-center space-x-2",children:[h.jsx("input",{ref:y,type:"text",value:o,onChange:l=>i(l.target.value),onKeyPress:l=>l.key==="Enter"&&E(),className:"flex-grow p-3 border border-border bg-surface rounded-lg h-12 text-lg focus:border-transparent focus:ring-2 focus:ring-brandPrimary",placeholder:"Type your message...",disabled:a}),h.jsx("button",{onClick:E,disabled:a||!o.trim(),className:"bg-brandPrimary text-white rounded-full h-12 w-12 flex items-center justify-center flex-shrink-0 disabled:bg-textMuted transition-colors","aria-label":"Send message",children:h.jsx("svg",{xmlns:"http://www.w3.org/2000/svg",className:"h-6 w-6",fill:"none",viewBox:"0 0 24 24",stroke:"currentColor",children:h.jsx("path",{strokeLinecap:"round",strokeLinejoin:"round",strokeWidth:2,d:"M12 19l9 2-9-18-9 18 9-2zm0 0v-8"})})})]})]})]})};export{ze as default};
