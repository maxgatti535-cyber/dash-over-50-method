...
const Progress: React.FC = () => {
  ...
  const handleDownloadReport = () => {
    const doc = new jsPDF();
    const profile = {
      name: getLocalStorageItem('profile.name', 'User'),
      age: getLocalStorageItem('profile.age', ''),
      weight: getLocalStorageItem('profile.weight', ''),
      conditions: getLocalStorageItem('profile.medicalConditions', 'None reported'),
    };

    // Style and Header
    doc.setFontSize(22);
    doc.setTextColor(27, 132, 124); // brandPrimary color
    doc.text('DASH Over 50 METHOD™', 14, 20);
    doc.setFontSize(16);
    doc.setTextColor(15, 23, 42); // textPrimary
    doc.text('Medical Report - Blood Pressure History', 14, 30);
    ...
    doc.text(`Age: ${profile.age}`, 80, 50);
    doc.text(`Weight: ${profile.weight}`, 130, 50);
    doc.text(`Medical Conditions: ${profile.conditions}`, 14, 58, { maxWidth: 180 });

    // BP Table
    const tableData = allBpReadings.map(r => {
      const cat = getBPCategory(Number(r.systolic), Number(r.diastolic));
      return [
        r.date,
        r.time,
    ...
    autoTable(doc, {
      startY: 70,
      head: [['Date', 'Time', 'Reading (mmHg)', 'Category', 'Notes']],
      body: tableData,
      headStyles: { fillColor: [27, 132, 124] },
    ...
    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text('Disclaimer: This report is for educational purposes only and is not a medical diagnosis.', 14, finalY + 15);
    doc.text('Please consult with your healthcare provider for evaluation.', 14, finalY + 20);

    doc.save(`dash_medical_report_${new Date().toISOString().split('T')[0]}.pdf`);
  };
    ...
    <div className="space-y-4">
      ...
      <div className="bg-surface p-5 rounded-xl shadow-sm shadow-shadowSoft border border-brandPrimaryDark">
        ...
        <button
          ...
          className="w-full mt-4 flex items-center justify-center gap-2 bg-brandPrimary text-white font-bold py-3 rounded-lg hover:bg-teal-800 transition-colors shadow-sm"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M4 17v2a2 2 0 0 0 2 2h12a2 2 0 0 0 2 -2v-2" /><path d="M7 11l5 5l5 -5" /><path d="M12 4l0 12" /></svg>
